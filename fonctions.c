...
void rechercher_mot(){
    ...
    scanf("%s", mot);
    FILE *f=fopen("dico.txt","r");//lire le dictionnaire en entier
    char str[MAX];
    while(fgets(str,MAX,f)!=NULL) {//tant que le fichier n'est pas fini
        char line[MAX];
        strcpy(line, str);
        char *deli = "\t";
        char *portion1 = strtok(line, deli);//couper la ligne au premier tab
        char *portion2 = strtok(NULL, deli);//couper la ligne au deucième tab
        char *portion3 = strtok(NULL, "\n");//couper la ligne au retour a la ligne
        if (strcmp(portion2,mot)==0){//comparer le mot saisit par l'utilisateur et les mots de base soit les mots de la deuxième colonne
