...
void copy_adjectif(char *arrive,char*depart)
{
...
s_tree Creer_arbre_adjectif(){
    ...
    copy_adjectif("adjectif.txt","dico.txt");
    FILE *f=fopen("adjectif.txt","r");
    char str[MAX];
    while(fgets(str,MAX,f)!=NULL) {
        char line[MAX];
        strcpy(line, str);
